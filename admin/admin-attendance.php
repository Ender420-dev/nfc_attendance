...
.btn-add {
  ...
  background: #2d89ef;       /* Primary blue */
  color: #fff;
  font-size: 15px;
  font-weight: 600;
  padding: 10px 18px;
...
.btn-add:hover {
  background: #1a5bbf;       /* Darker blue on hover */
...
<div id="attendanceModal" class="modal">
  <div class="modal-content">
    ...
    <div id="employeeInfo" style="margin:10px 0; font-weight: bold; color: #2d89ef;">
...
<input type="datetime-local" name="scanTime" id="scanTime">


      <input type="hidden" name="remarks" id="remarks" placeholder="Manual Entry" value="Manual Entry" >
...
function setCurrentDateTime() {
  ...
  document.getElementById("scanTime").value = `${year}-${month}-${day}T${hours}:${minutes}`;
}

// Fetch Employee Info (AJAX)
...
attendanceForm.addEventListener("submit", function(e){
  ...
  .then(data => {
    if(data.success){
      ...
      const scanDate = new Date(formData.get("scanTime"));
...
(function initClock(){
  ...
  function updateClock(){
    const now = new Date();
    const dateStr = now.toLocaleDateString(undefined,{ weekday:"long", year:"numeric", month:"long", day:"2-digit"});
